	...
	else
	{
		...
		if(damage > 0)
		{
			if(target->hasCondition(CONDITION_MANASHIELD) && params.combatType != COMBAT_UNDEFINEDDAMAGE)
			{
				...
				elementDamage = 0; // TODO: I don't know how it works ;( Vizjus 566566 add Items that add element damage!
			...
			// item->attribute->elementAddFire >= 0
			// elementDamage = elementDanage + (elementAddFire * 0.01)
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, int32_t manaChange, CombatType_t combatType/* = COMBAT_MANADRAIN*/, bool inherited/* = false*/)
{
	const Position& targetPos = target->getPosition();
	if(manaChange > 0)
	{
