...
ChatChannel::ChatChannel(uint16_t id, const std::string& name, uint16_t flags, uint32_t access/* = 0*/, uint32_t level/* = 1*/, Condition* condition/* = NULL*/, int32_t conditionId/* = -1*/, const std::string& conditionMessage/* = ""*/, VocationMap* vocationMap/* = NULL*/):
	...
	uint32_t statementId, bool anonymous/* = false*/)
{
		...
		if(channelId == CHANNEL_GUILD)
		{
			switch(player->getGuildLevel())
			{
				case GUILDLEVEL_VICE:
					return channel->talk(player, MSG_CHANNEL_O, text, statementId);
				case GUILDLEVEL_LEADER:
					return channel->talk(player, MSG_CHANNEL_RN, text, statementId);
				default:
					break;
			}
		}
	...
	if(text.substr(1) == "disband")
	{
		if(player->getGuildLevel() == GUILDLEVEL_LEADER)
		{
			...
			channel->talk("", MSG_CHANNEL_RN, "The guild has been disbanded.");
		}
		else
				...
				if(g_game.getPlayerByNameWildcard(param, paramPlayer) == RET_NOERROR)
				{
					if(paramPlayer->getGuildId() == 0)
					{
						if(!paramPlayer->isGuildInvited(player->getGuildId()))
						{
							...
							channel->talk("", MSG_CHANNEL_O, buffer);
							...
							else if(text[2] == 'a')
							{
								if(paramPlayer->getGuildLevel() == GUILDLEVEL_VICE)
								{
									...
									if(paramPlayer->getLevel() >= levelToFormGuild)
									{
										...
										channel->talk("", MSG_CHANNEL_RN, buffer);
									}
									else
									{
	...
	else if(text.substr(1, 9) == "cleanmotd")
	{
		...
		player->sendChannelMessage("", "Guild commands with parameters: disband, invite[name], leave, kick[name], revoke[name], demote[name], promote[name], passleadership[name], nick[name, nick], setrankname[oldName, newName], setmotd[text] and cleanmotd.", MSG_CHANNEL_O, CHANNEL_GUILD);
	else
		return false;
