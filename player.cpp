...
bool Player::onDeath()
{ //prevent loss odpowiada za utrate EQ np aol
	Item *preventLoss = NULL, *preventDrop = NULL;
	// jezeli graczjest w pvp zone nie traci itemow
	if(getZone() == ZONE_HARDCORE)
	{
		//silnik utawia drop rate na 0 dla gracza
		setDropLoot(LOOT_DROP_NONE);
		//silnik ustawia drop skill na 0 dla gracza
		setLossSkill(false);
	} //redskull system pomija prevent drop z racji na red skulla
