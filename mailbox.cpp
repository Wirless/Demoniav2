...
ReturnValue Mailbox::canSend(const Item* item, Creature* actor) const
{
	...
	if(actor)
	{
		if(Player* player = actor->getPlayer())
		{
			...
			{
				if(Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_MUTED, g_config.getNumber(ConfigManager::MAIL_BLOCK), 0, false, 2))
				{
...
ReturnValue Mailbox::__queryAdd(int32_t, const Thing* thing, uint32_t, uint32_t, Creature* actor/* = NULL*/) const
{
